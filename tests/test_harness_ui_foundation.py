...
from hermes_harness import bff as harness
from hermes_harness import server as harness_server
...
def test_harness_is_enabled_by_standalone_entrypoint_and_can_be_disabled_explicitly():
    ...
    # The legacy WebUI flag was removed in the standalone repo: Harness is
    # enabled by default and only HERMES_HARNESS_ENABLED can disable it.
    assert harness.harness_enabled({"HERMES_WEBUI_HARNESS_UI": "1"}) is True
    assert harness.harness_enabled({"HERMES_WEBUI_HARNESS_UI": "0"}) is True
    assert "HERMES_WEBUI_HARNESS_UI" not in (ROOT / "hermes_harness" / "bff.py").read_text(encoding="utf-8")
...
def test_static_client_contains_no_gateway_bearer_secret_surface():
    assets = [
        ROOT / "hermes_harness" / "static" / "harness.html",
        ROOT / "hermes_harness" / "static" / "harness.js",
        ROOT / "hermes_harness" / "static" / "harness-preferences.js",
        ROOT / "hermes_harness" / "static" / "harness-models.js",
    ]
...
def test_browser_projection_is_explicitly_bounded_and_event_driven():
    js = (ROOT / "hermes_harness" / "static" / "harness.js").read_text(encoding="utf-8")
    prefs = (ROOT / "hermes_harness" / "static" / "harness-preferences.js").read_text(encoding="utf-8")
...
def test_standalone_server_preserves_auth_csrf_and_guards_remote_bind(monkeypatch):
    source = (ROOT / "hermes_harness" / "server.py").read_text(encoding="utf-8")
    auth_source = (ROOT / "hermes_harness" / "auth.py").read_text(encoding="utf-8")
...
def test_harness_does_not_modify_legacy_server_entrypoint():
    # In the standalone repository there is no legacy WebUI server entrypoint
    # to modify: the WebUI tree (server.py, api/, harness_runtime/) is absent.
    assert not (ROOT / "server.py").exists()
    assert not (ROOT / "api").exists()
    assert not (ROOT / "harness_runtime").exists()
