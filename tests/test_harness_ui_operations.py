...
def test_h4_server_preserves_operations_bff_through_h5_layering():
    harness_server = (ROOT / "hermes_harness" / "server.py").read_text(encoding="utf-8")
    h5_recovery = (ROOT / "hermes_harness" / "recovery.py").read_text(
        encoding="utf-8"
    )
    ...
    assert "from hermes_harness.recovery import" in harness_server
    assert "from hermes_harness import operations" in h5_recovery
    assert '"/harness-operations.js"' in (ROOT / "hermes_harness" / "operations.py").read_text(encoding="utf-8")
    assert "serve_harness_asset(self, parsed.path)" in harness_server
    # The standalone repo has no legacy WebUI server entrypoint.
    assert not (ROOT / "server.py").exists()


def test_h4_browser_asset_exposes_cancel_retry_without_backend_secrets():
    source = (ROOT / "hermes_harness" / "static" / "harness-operations.js").read_text(encoding="utf-8")
